...
Lastly we merge all three data sources (rats, rat predictors, ZIP geometry) into a single data frame called `rats_all`.
...
The function `poly2nb()` from the **spdep** package takes our data with spatial geometry and returns a neighbor adjacency graph. This tells us, for each ZIP code in our data, which ZIP codes touch it at at least one point. We could take a look at this graph using the base `plot(zips_adj)` method, but I prefer the look and flexibility of ggplot.
